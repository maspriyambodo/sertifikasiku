...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sys_users_insert` (IN `user_name` VARCHAR(100), IN `pass_word` VARCHAR(100), IN `id_role` INT, IN `avatar` VARCHAR(100), IN `status_active` INT, IN `user_login` INT, IN `nama_lengkap` VARCHAR(100), IN `telepon` VARCHAR(100))  BEGIN
...
IF pass_word = 'update' THEN 
		...
		UPDATE 
			sys_users 
		SET 
			sys_users.uname = user_name, 
			sys_users.role_id = id_role, 
			sys_users.pict = avatar, 
			sys_users.sysupdateuser = user_login, 
			sys_users.sysupdatedate = NOW() 
		WHERE 
			sys_users.id = status_active;
			
			UPDATE 
				dt_users
			SET 
				dt_users.nama = nama_lengkap,
				dt_users.telp = telepon,
				dt_users.sysupdateuser = user_login,
				dt_users.sysupdatedate = NOW()
			WHERE
				dt_users.sys_user_id = status_active;
	
ELSE

		INSERT INTO `sys_users`(
			sys_users.uname, sys_users.pwd, sys_users.role_id, 
			sys_users.pict, sys_users.stat, 
			sys_users.syscreateuser, sys_users.syscreatedate
		) 
		VALUES 
			(
				user_name,
